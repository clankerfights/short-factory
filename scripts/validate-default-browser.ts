...
const packet = parseClipFactoryPacketWire({
  version: 1,
  clipId: "default-browser-cue",
  clipUrl: "https://clankerfights.ai/clip/default-browser-cue",
  sourceUrl: "https://clankerfights.ai/clip/default-browser-cue",
  playbackUrl: "https://clankerfights.ai/?clip=default-browser-cue&factory=1",
  apiUrl: "https://clankerfights.ai/api/clips/default-browser-cue",
  game: "browser-test",
  gameRevisionId: "rev-default-browser",
  durationSeconds: 6,
  trimStartMs: 0,
  trimEndMs: 6000,
...
validateDefaultBrowserTiming()
  .then(() => {
    console.log("Default template browser timing validation passed.");
  })
  .catch((error: unknown) => {
    console.error(error);
    process.exitCode = 1;
...
async function validateDefaultBrowserTiming(): Promise<void> {
  assert.doesNotThrow(() =>
  ...
  try {
    ...
    for (const speed of [1, 2, 16, 32]) {
      ...
      const composition = await applyDefaultTemplate(speedJob, { ...variant });
...
async function writeDefaultSpeechStub(args: {
  ...
  await fs.writeFile(absolutePath, "default browser validation audio stub");
}
