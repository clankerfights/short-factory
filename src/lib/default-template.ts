...
export async function applyDefaultTemplate(
  ...
  if (isCurrentDefaultTemplate(variant.composition)) {
  ...
  const sourceDurationSeconds = Math.max(
    ...
    finalHighlightedMessage
      ? finalHighlightedMessage.timeStart +
          DEFAULT_TEMPLATE_TIMING.finalHighlightPostrollSeconds
      : 0,
    detectedCueDurationSeconds
      ? detectedCueDurationSeconds + DEFAULT_TEMPLATE_TIMING.finalHighlightPostrollSeconds
      : 0,
  );
  ...
  const playback = {
    speed: job.quoteJob.clipPlaybackSpeed ?? DEFAULT_TEMPLATE_TIMING.gameplaySpeed,
  };
...
export function isCurrentDefaultTemplate(
  composition: EditComposition | undefined,
): composition is EditComposition {
  return (
    composition?.templateId === DEFAULT_TEMPLATE_ID &&
    composition.templateVersion === DEFAULT_TEMPLATE_VERSION
  );
}
